...
const HomeView = ({ navigateTo, openAlbumBySlug, openEvent }) => {
  ...
  return (
    <div className="w-full overflow-x-hidden bg-[#F4F4F3]">
      ...
      {/* --- HERO SECTION --- */}
      <div className="relative min-h-screen flex flex-col justify-center border-b-2 border-[#041E42] overflow-hidden antialiased">
        <img 
            src="/og-image.jpg"
            alt="Alumni singing"
            fetchPriority="high"
        ...
        <div className="max-w-[1920px] mx-auto w-full flex flex-col items-center justify-center relative z-10 py-12 px-4 md:px-12">
          {/* Typography Stack - [text-wrap:balance] ensures even distribution */}
          <div className="w-full flex flex-col items-center justify-center text-center select-none mb-20 md:mb-24 leading-none text-[#041E42] [text-wrap:balance]">
            <h1 className="w-full text-[12vw] font-serif leading-[0.75] tracking-tighter relative z-10">
              BROTHERHOOD
            </h1>
            <h1 className="w-full text-[18vw] font-serif leading-[0.75] tracking-tighter opacity-40 italic -mt-[3vw] relative z-0">
              HARMONY
            </h1>
            <h1 className="w-full text-[17.5vw] font-serif leading-[0.75] tracking-tighter block -mt-[4vw] relative z-10">
              HISTORY
            </h1>
          </div>

          {/* Action Buttons */}
          <div className="flex flex-col md:flex-row gap-8 md:gap-24 items-center z-20">
             <button 
                onClick={() => openAlbumBySlug('desperate-chimes-desperate-measures')} 
                className="group relative text-[11px] font-sans font-bold tracking-[0.1em] uppercase text-[#041E42] hover:text-[#D50032] transition-colors"
             >
            ...
            {ctmAlumni && (
              <button 
                  ...
                  className={`group relative text-[11px] font-sans font-bold tracking-[0.1em] uppercase transition-colors ${
                    isPast(ctmAlumni?.date) ? 'text-[#041E42]/70 cursor-default' : 'text-[#041E42] hover:text-[#D50032]'
                  }`}
              >
                  {isPast(ctmAlumni?.date) ? 'Event Archived' : 'Book Cherry Tree Tickets'}
                  {!isPast(ctmAlumni?.date) && (
                    <span className="absolute -bottom-2 left-0 w-full h-[2px] bg-[#041E42] group-hover:bg-[#D50032] transition-colors"></span>
                  )}
              </button>
            )}
      ...
      {/* --- DIRECTORY SECTION --- */}
      <section className="py-24 md:py-32 px-6 md:px-12 text-[#041E42] antialiased">
